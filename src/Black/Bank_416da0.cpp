...
__declspec(naked) bool32_t Living::IsPoisoned()
{
    __asm {
            xor eax, eax
            mov al, byte ptr [ecx + 0x000000b4]
            shr eax, 1
            and al, 0x01
            ret
        }
...
__declspec(naked) void Living::SetPoisoned(int param_1)
{
    __asm {
            xor eax, eax
            mov al, byte ptr [esp + 0x04]
            xor edx, edx
            mov dx, word ptr [ecx + 0x000000b4]
            and eax, 0x01
            shl eax, 1
            and edx, 0x0000fffd
            or eax, edx
            mov word ptr [ecx + 0x000000b4], ax
            ret 4
        }
}
...
{
    return (*(uint8_t*)((char*)this + 0xb4) >> 6) & 1;
}

// win1.41 00417000 mac 1012f850 Living::SetSkeleton(int)
__declspec(naked) void Living::SetSkeleton(int param_1)
{
    __asm {
            xor eax, eax
            mov al, byte ptr [esp + 0x04]
...
{
    *(LH3DColor*)((char*)this + 0x000000d0) = param_1;
}

// win1.41 004174a0 mac inlined Villager::MoveAllowedForChessGame(void)
__declspec(naked) bool32_t Villager::MoveAllowedForChessGame()
{
    __asm {
            xor eax, eax
            ret 8
...
__declspec(naked) bool32_t Villager::AttackAllowedForChessGame()
{
    __asm {
            xor eax, eax
            ret 8
...
{
    return *(int*)(*(char**)((char*)this + 0x28) + 0x00000200);
}
...
{
    return (DEATH_REASON)*(uint8_t*)((char*)this + 0x10c);
}
...
{
    *(uint32_t*)((char*)this + 0x20) = (uint32_t)child;
}
...
__declspec(naked) bool32_t Animal::ValidForPlaceInHand(GInterfaceStatus* param_1)
{
    __asm {
            mov eax, dword ptr [ecx + 0x28]
            mov edx, dword ptr [eax + 0x00000228]
            xor ecx, ecx
            test edx, edx
            setne cl
            mov eax, ecx
...
bool32_t PuzzleHorse::ArrivesAtWoodReaction()
{
    return (bool32_t)0x1;
}


bool32_t PuzzleHorse::IsFoodSpeedUp()
{
    __asm { xor al, al }
}
...
int PuzzleHorse::CallOutofAnimationFunction(VILLAGER_STATES param_1)
{
    return (int)0xffffffff;
}

// ==== AUTOHARVEST ====
// AUTOHARVEST ?SetFoodSpeedup@PuzzleHorse@@UAEX_N@Z
void PuzzleHorse::SetFoodSpeedup(bool param_1)
{
}
